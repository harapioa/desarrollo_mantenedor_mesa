...
QUERY_CASOS_PD_ENTIDAD = """
    SELECT 
        ...
        analista_paterno,
        analista_materno,
        entidad_ejecutora_nombre,
        sector_estrategico
    FROM ( SELECT DISTINCT caso.caso_id,
                        proc.proc_id                     proc_id,
                        proc.proc_numero_siad
                        || '.'
                        || caso.caso_numero              nro_siad,
                        CASE
                        WHEN proc.proc_numero_rpd IS NULL THEN
                        NULL
                        ELSE
                ...
                LEFT JOIN (SELECT acvi.proc_id,
                                    LISTAGG(acvi.tipo_producto
                                            ...
                                            || acvi.unidad_origen
                                            || '; ', CHR(10)) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) lista_productos_caso,
                                    LISTAGG(acvi.tipo_producto || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) prod_tipos,
                                    LISTAGG(acvi.producto_numero || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) prod_numeros,
                                    LISTAGG(acvi.producto_periodo || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) prod_periodos,
                                    LISTAGG(acvi.producto_fecha || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) prod_fechas,
                                    LISTAGG(acvi.contexto || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) prod_contextos,
                                    LISTAGG(acvi.unidad_origen || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                                                                        ORDER BY acvi.proc_id) prod_unidades
                                                                                        FROM siad_glob.acfi_vinculadas_v1_3 acvi
                                                                            WHERE acvi.proc_id IS NOT NULL
                                                                GROUP BY acvi.proc_id
                            UNION
                            SELECT avpo.proc_id,
                                    LISTAGG(avpo.tipo_producto
                                            || ';'
                                            || avpo.producto_numero
                                            || '/'
                                            || avpo.producto_periodo
                                            || '; '
                                            || avpo.unidad_origen
                                            || '; ', CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) lista_productos_caso,
                                    LISTAGG(avpo.acfi_tipo || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) prod_tipos,
                                    LISTAGG(avpo.producto_numero || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) prod_numeros,
                                    LISTAGG(avpo.producto_periodo || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) prod_periodos,
                                    LISTAGG(avpo.producto_fecha || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) prod_fechas,
                                    LISTAGG(avpo.contexto || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) prod_contextos,
                                    LISTAGG(avpo.unidad_origen || CHR(10) ON OVERFLOW TRUNCATE WITH COUNT) WITHIN GROUP(
                                    ORDER BY avpo.proc_id) prod_unidades
                            FROM siad_glob.acfi_vinculadas_prof_v1_1 avpo
                --WHERE avpo.proc_id IS NOT NULL
                            GROUP BY avpo.proc_id
                )                                       prod_orig ON prod_orig.proc_id = proc.proc_id
                LEFT JOIN ( SELECT reso_inicio.*
                                        FROM sica_wl_1.eeprdi_resoluciones_pdis_v1_0 reso_inicio
                            WHERE reso_inicio.acti_tipo = 'RESOLUCION_INICIO'
                )                                       resolucion_inicio ON pdis.pdis_id = resolucion_inicio.pdis_id
                LEFT JOIN ( SELECT reso_contralor.pdis_id,
                                    LISTAGG(reso_contralor.resolucion_con_tipo, CHR(10)) WITHIN GROUP(
                                                    ORDER BY reso_contralor.pdis_id ASC) "RESO_CONTRALOR"
                                                    FROM sica_wl_1.eeprdi_resoluciones_pdis_v1_0 reso_contralor
                                        WHERE reso_contralor.acti_tipo IN ( 'RESOLUCION_CGR_EXENTA', 'RESOLUCION_CGR_AFECTA' )
                            GROUP BY reso_contralor.pdis_id
                )                                       resolucion_contralor ON pdis.pdis_id = resolucion_contralor.pdis_id
                LEFT JOIN ( SELECT reso_recurso.pdis_id,
                                    LISTAGG(reso_recurso.resolucion_con_tipo, CHR(10)) WITHIN GROUP(
                                                    ORDER BY reso_recurso.pdis_id ASC) "RESO_RECURSO"
                                                    FROM sica_wl_1.eeprdi_resoluciones_pdis_v1_0 reso_recurso
                                        WHERE reso_recurso.acti_tipo IN ( 'RES_RECURSO_JERARQUICO', 'RES_RECURSO_REPOSICION' )
                            GROUP BY reso_recurso.pdis_id
                )                                       resolucion_recurso ON pdis.pdis_id = resolucion_recurso.pdis_id
                LEFT JOIN ( SELECT reen.caso_id,
                                    LISTAGG(reen.reen_oficio_numero
                                            || '; '
                                            || to_char(reen.reen_oficio_fecha, 'DD-MM-YYYY')
                                            || '; '
                                            || upper(TRIM(enti_dest.ensv_nombre))
                                            || '; '
                                            || upper(TRIM(dest.dest_cargo)), CHR(10)) WITHIN GROUP(
                                                    ORDER BY reen.caso_id ASC) "OFICIOS_REMISION"
                                                    FROM siad_glob.siad_remision_exp_entidad reen
                                        ...
                                        WHERE reen.reen_vigencia = 'VIGENTE'
                            GROUP BY reen.caso_id
                )                                       oficio_remision ON caso.caso_id = oficio_remision.caso_id
                LEFT JOIN ( SELECT aafi.caso_id,
                                    LISTAGG(aafi.aafi_tipo
                                            || '; '
                                            || aafi.aafi_numero
                                            || '; '
                                            || to_char(aafi.aafi_fecha, 'DD-MM-YYYY')
                                            || '; '
                                            || aafi.aafi_conclusion
                                            || '; '
                                            || aafi.aafi_nombre_descripcion, CHR(10)) WITHIN GROUP(
                                                    ORDER BY aafi.caso_id DESC, aafi.aafi_fecha DESC) "ACTOS_ADM_TERMINO"
                                                    FROM siad_glob.siad_acto_administrativo_fin aafi
                                                    INNER JOIN siad_glob.siad_caso caso ON caso.caso_id = aafi.caso_id
                                        WHERE caso.caso_vigencia = 'VIGENTE'
                                                AND aafi.aafi_estado = 'ACEPTADA'
                                                AND aafi.aafi_vigencia = 'VIGENTE'
                            GROUP BY aafi.caso_id
                )                                       acto_adm_termino ON acto_adm_termino.caso_id = caso.caso_id
                LEFT JOIN sica_wl_1.eeprdi_resoluciones_pdis_v1_0 reso_cgr_exenta ON pdis.pdis_id = reso_cgr_exenta.pdis_id
                                                                                    AND reso_cgr_exenta.acti_tipo IN ( 'RESOLUCION_CGR_EXENTA'
                                                                                    , 'RESOLUCION_CGR_AFECTA' )
                LEFT JOIN ( SELECT orof.pdis_id                  pdis_id,
                                    LISTAGG(orof.orof_origen
                                            || '; '
                                            || orof.orof_fuente
                                            || '; '
                                            || to_char(orof.orof_fecha, 'DD-MM-YYYY'), CHR(10)) WITHIN GROUP(
                                        ORDER BY orof.orof_fecha ASC) lista_origen_oficio
                                        FROM own_eeprdi.eeprdi_origen_oficio orof
                            GROUP BY orof.pdis_id
                ...
                LEFT JOIN ( SELECT acti_vifi.pdis_id,
                                    vista_fiscal.vifi_resp_administrativa resp_adm_alcalde_prop
                                        FROM own_eeprdi.eeprdi_vista_fiscal       vista_fiscal
                            ...
                            WHERE acti_vifi.acti_tipo = 'VISTA_FISCAL'
                                    AND acti_vifi.acti_estado IN ( 'APROBADA', 'FINALIZADA' )
                                    AND vista_fiscal.vifi_id = ( SELECT MAX(vifi1.vifi_id)
                                                                                        FROM own_eeprdi.eeprdi_vista_fiscal vifi1
                                                                                        LEFT JOIN own_eeprdi.eeprdi_actividad    acti_vifi1
                                                                                        ON ( acti_vifi1.acti_id = vifi1.acti_id )
                                                        WHERE acti_vifi1.acti_tipo = 'VISTA_FISCAL'
                ...
                LEFT JOIN ( SELECT caso.caso_id,
                                    tiac.tiac_nombre,
                                    actu.actu_estado,
                                    LISTAGG(actu.actu_numero
                                            || '; '
                                            || to_char(actu.actu_fecha, 'DD-MM-YYYY'), CHR(10)) WITHIN GROUP(
                                                    ORDER BY actu.actu_fecha ASC) lista_actos
                                                    FROM siad_glob.siad_caso caso
                                        ...
                                        WHERE tiac.tiac_nombre = 'COPIA_ACTO_ADMINISTRATIVO_DE_INICIO'
                ...
                LEFT JOIN ( SELECT caso.caso_id,
                                    ...
                                    LISTAGG(actu.actu_numero
                                            || '; '
                                            || to_char(actu.actu_fecha, 'DD-MM-YYYY')
                                            || ': '
                                            || actu.actu_tipo_documento, CHR(10)) WITHIN GROUP(
                                                    ORDER BY actu.actu_fecha ASC) lista_actos
                                                    FROM siad_glob.siad_caso caso
                                                    INNER JOIN siad_glob.siad_tipo_actuacion tiac ON caso.caso_id = tiac.caso_id
                                                    INNER JOIN siad_glob.siad_actuacion      actu ON actu.tiac_id = tiac.tiac_id
                                        WHERE tiac.tiac_nombre = 'COPIA_ACTO_ADMINISTRATIVO_DE_REAPERTURA'
                                                AND actu.actu_estado IN ( 'ACEPTADA', 'APROBADA' )
                            GROUP BY caso.caso_id,
                                    tiac.tiac_nombre,
                                    actu.actu_estado
                )                                       acto_reapertura_caso ON acto_reapertura_caso.caso_id = caso.caso_id
                LEFT JOIN ( SELECT caso.caso_id,
                                    aafi.aafi_estado,
                                    aafi.aafi_tipo,
                                    LISTAGG(aafi.aafi_numero
                                            || '; '
                                            || to_char(aafi.aafi_fecha, 'DD-MM-YYYY'), CHR(10)) WITHIN GROUP(
                                                    ORDER BY aafi.aafi_fecha) lista_actos
                                                    FROM siad_glob.siad_caso caso
                                                    INNER JOIN siad_glob.siad_acto_administrativo_fin aafi ON caso.caso_id = aafi.caso_id
                                        WHERE aafi.aafi_estado IN ( 'ACEPTADA', 'APROBADA' )
                                                AND aafi.aafi_tipo = 'EXENTO'
                            GROUP BY caso.caso_id,
                                    aafi.aafi_estado,
                                    aafi.aafi_tipo
                )                                       acto_termino_exento_caso ON acto_termino_exento_caso.caso_id = caso.caso_id
                LEFT JOIN ( SELECT caso.caso_id,
                                    aafi.aafi_estado,
                                    aafi.aafi_tipo,
                                    LISTAGG(aafi.aafi_numero
                            ...
                            GROUP BY caso.caso_id,
                                    aafi.aafi_estado,
                                    aafi.aafi_tipo
                )                                       acto_termino_afecto_caso ON acto_termino_afecto_caso.caso_id = caso.caso_id
        WHERE caso.caso_vigencia = 'VIGENTE'
                AND caso.caso_categoria IN ( 'PD_ENTIDAD', 'PD_CGR' )
